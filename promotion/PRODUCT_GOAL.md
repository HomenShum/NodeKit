...
| 8 | Web-quality audit: no major unresolved | PASS | Both halves ran and both tools were retained. **Lighthouse 13.4.1** (`npx --yes lighthouse@13.4.1 <url> --output=json --output-path=... --chrome-flags="--headless"`): accessibility **1.00**, best-practices **1.00**, SEO **1.00**, performance 0.99–1.00. **axe-core CLI 4.13.0** (`npx --yes @axe-core/cli@4.13.0 <url> --save ...`): **0 violations**. **Plus a 60-cell sweep** neither CLI can do, because each audits one page in one colour scheme: 15 states x 2 schemes x 2 widths, **0 violations at every impact level**, not merely at the serious/critical ones the producer gates on. Core Web Vitals, quoted as ranges over five runs on this machine because two of the three move run to run: CLS **0.054** and TBT **0 ms** were identical in every run; LCP fell between **1208 and 1379 ms**, FCP between 1054 and 1103 ms — all inside the "good" boundaries, which is what the producer actually asserts (`thresholds` in the artifact), rather than any single number being a property of the tree. The same producer on the pre-fix tree exits 1 with accessibility 0.95, best-practices 0.96, one console error and 28 violating cells — committed beside it as `evidence/web-quality/before/`, so before and after come from one instrument. Producer `npm run promotion:web-quality`, confirmed to run from a cold `nodekit create` with no arguments; artifacts `evidence/web-quality/{lighthouse-app,axe-app,axe-sweep,web-quality}.json` |
...
| 10 | Performance does not obstruct interaction | PASS | Two independent measurements. Wall-clock from click or keypress to the asserted DOM text: confirm 28 ms, propose 77 ms, approve 56 ms (`evidence/capture-report.json`, `timing`). Lighthouse 13.4.1 under simulated mobile throttling: total blocking time **0 ms in every run**, CLS 0.054, LCP 1208–1379 ms, performance 0.99–1.00 (`evidence/web-quality/lighthouse-app.json`, `evidence/web-quality/web-quality.json`). Nothing on the main thread blocks input; TBT is the number that would show it if anything did, and it is zero |
