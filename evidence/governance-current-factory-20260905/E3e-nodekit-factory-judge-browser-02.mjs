...
const P=import.meta.dirname,S='D:/VSCode Projects/node-platform/.worktrees/project-consolidation-20260904',C='C:/Users/hshum/AppData/Local/Temp/nodekit-factory-acceptance-zswER5/measured-journey/domain-blank-base',out=path.join(P,'E3e-nodekit-factory-independent-browser-02');
...
try{
 ...
 await capture(page,'running-mobile-controls-390'); const initial=await (await fetch(base+'/api/state')).json();await page.locator('#mobile-propose').click();await page.locator('body[data-scenario="proposal_pending"]').waitFor();await page.locator('#mobile-reject').click();await page.locator('body[data-scenario="proposal_rejected"]').waitFor();const rejected=await (await fetch(base+'/api/state')).json();check(hash(initial.artifact)===hash(rejected.artifact)&&rejected.receipt===null,'real reject preserves accepted artifact and creates no receipt');
 await page.locator('#mobile-propose').click();await page.locator('#mobile-approve').click();await page.locator('#completion').waitFor({state:'visible'});await capture(page,'accepted-390');
