...
/** Deterministic retrieval: token overlap between the question and each graph node's content. */
export function answer(store, question) {
  const queryTokens = new Set(tokenize(question));
  const matches = [];
  for (const node of store.nodes) {
    if (typeof node.content !== "string" || node.content === "") continue;
    const nodeTokens = new Set(tokenize(node.content));
    const overlap = [...queryTokens].filter((token) => nodeTokens.has(token));
    if (overlap.length > 0) {
      matches.push({
        excerpt: node.content.slice(0, 280),
        binding: { noteId: node.sourceId, digest: node.sourceDigest },
  ...
  return {
    ...
    ...(top.length === 0
      ? { unboundReason: store.nodes.length === 0
          ? "the store holds zero nodes; nothing exists to bind an answer to"
          : "no stored node matches the question; an unmatched answer would cite nothing" }
      : {}),
  };
}
...
function page(store, result) {
  const counts = `${store.nodes.length} nodes, ${store.relations.length} relations, ${store.relationTypes.length} relation types, ${store.relationLists.length} list entries`;
  const body = result
    ? result.unbound
...
<body data-nodekit-artifact="mew-migration-agent-surface" data-store-nodes="${store.nodes.length}">
