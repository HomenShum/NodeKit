...
5. For an empty target, run `nodekit create --local-proof`; add `--package-manager pnpm` when pnpm is available and appropriate. For an existing target, read the [existing-product adoption field card](references/existing-product-adoption.md), generate the active behavior inventory, then run `nodekit adopt` and inspect its collision receipt before accepting changes. Migrate one slice in place; do not generate a replacement shell before parity.
